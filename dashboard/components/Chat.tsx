  ...
  return (
    ...
    >
      <p className="mb-3 text-[11px] leading-relaxed text-slate-500">
        ...
        <span className="font-mono text-slate-300">assistant</span> in the table above —
        it answers by issuing syscalls through the kernel, not by calling the API from
        outside. Every read it makes is listed under its answer and appears in the live
        syscall trace.
      </p>

      {dead && (
        <div className="mb-3 rounded border border-rose-500/40 bg-rose-500/10 p-3">
          <p className="text-xs font-semibold text-rose-300">
            process &lsquo;{status?.pid}&rsquo; is {status?.state ?? "gone"}
          </p>
          <p className="mt-1 text-[11px] leading-relaxed text-slate-400">
            The assistant was terminated, so it can no longer issue syscalls. This is the
            kernel working as intended: it is an ordinary process and can be killed like
            any other, from the shell with{" "}
      ...
      <div className="mb-3 space-y-3">
        {turns.length === 0 && !dead && (
          <p className="text-xs text-slate-500">
            Try: “what processes are running?” · “what did the starvation benchmark
            find?” · “what is in researcher&rsquo;s memory?”
          </p>
        )}
        {turns.map((t, i) => (
          <div key={i}>
      ...
      {error && <p className="mb-2 text-xs text-rose-400">error: {error}</p>}
      ...
      <div className="flex gap-2">
        <input
          ...
          disabled={dead || busy}
          placeholder={dead ? "process terminated" : "ask about the running kernel…"}
        ...
        <button
          ...
          className="shrink-0 rounded border border-slate-700 bg-slate-800/60 px-3 py-1.5 text-xs text-slate-200 hover:bg-slate-700/60 disabled:opacity-40"
        >
          send
        </button>
      </div>
